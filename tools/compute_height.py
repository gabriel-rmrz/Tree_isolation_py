...
def compute_height(P, inputs):
  ...
  sq = inputs["HeightSquare"] # edge lenth of squares used in bottom definition
  ...
  if DEBUG:
    ...
    print(f"len(Tri.simplices): {len((Tri.simplices)[:,0])}")
    print(f"a: {a}")
    print(f"SQ: {SQ}")
  t = 0
  for i in range(1, n+1):
    ...
    V1 = Q[1,:]- Q[0,:]
    V2 = Q[2,:]- Q[0,:]
    n1 = np.ceil(np.linalg.norm(V1)/SQ*2)
    n2 = np.ceil(np.linalg.norm(V2)/SQ*2)
    if DEBUG:
      print(f"V1: {V1}")
      print(f"V2: {V2}")
      print(f"n1: {n1}")
